...
import mixprop.data.utils
from mixprop.data import set_cache_mol, empty_cache
from mixprop.features import get_available_features_generators
...
class CommonArgs(Tap):
    ...
    def features_scaling(self) -> bool:
        """
        Whether to apply normalization with a :class:`~mixprop.data.scaler.StandardScaler`
...
class TrainArgs(CommonArgs):
    """:class:`TrainArgs` includes :class:`CommonArgs` along with additional arguments used for training a mixprop model."""
    ...
    with mixprop's default molecule or multi-molecule encoders, or in :code:`reaction_solvent` mode where it applies to the solvent only.
    """
    ...
    def process_args(self) -> None:
        ...
        self.smiles_columns = mixprop.data.utils.preprocess_smiles_columns(
            path=self.data_path,
...
class PredictArgs(CommonArgs):
    """:class:`PredictArgs` includes :class:`CommonArgs` along with additional arguments used for predicting with a mixprop model."""
...
class InterpretArgs(CommonArgs):
    """:class:`InterpretArgs` includes :class:`CommonArgs` along with additional arguments used for interpreting a trained mixprop model."""
...
class HyperoptArgs(TrainArgs):
    """:class:`HyperoptArgs` includes :class:`TrainArgs` along with additional arguments used for optimizing mixprop hyperparameters."""
