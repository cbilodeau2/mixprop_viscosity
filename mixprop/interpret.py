...
from mixprop.args import InterpretArgs
from mixprop.data import get_data_from_smiles, get_header, get_smiles, MoleculeDataLoader, MoleculeDataset
from mixprop.train import predict
from mixprop.utils import load_args, load_checkpoint, load_scalers, timeit
...
class mixpropModel:
    """A :class:`mixpropModel` is a wrapper around a :class:`~mixprop.models.model.MoleculeModel` for interpretation."""

    def __init__(self, args: InterpretArgs) -> None:
        """
        :param args: A :class:`~mixprop.args.InterpretArgs` object containing arguments for interpretation.
        """
        self.args = args
...
def mcts_rollout(node: MCTSNode,
    ...
    :param scoring_function: A function for scoring subgraph SMILES using a mixprop model.
...
def interpret(args: InterpretArgs) -> None:
    """
    Runs interpretation of a mixprop model using the Monte Carlo Tree Search algorithm.
    ...
    mixprop_model = mixpropModel(args)

    def scoring_function(smiles: List[str]) -> List[float]:
        return mixprop_model(smiles)[:, args.property_id - 1]
...
def mixprop_interpret() -> None:
    """Runs interpretation of a mixprop model.

    This is the entry point for the command line command :code:`mixprop_interpret`.
    """
