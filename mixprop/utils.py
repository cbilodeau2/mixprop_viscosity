...
from mixprop.args import PredictArgs, TrainArgs, FingerprintArgs
from mixprop.data import StandardScaler, MoleculeDataset, preprocess_smiles_columns, get_task_names
from mixprop.models import MoleculeModel
from mixprop.nn_utils import NoamLR
...
def save_checkpoint(path: str,
    ...
    :param model: A :class:`~mixprop.models.model.MoleculeModel`.
    :param scaler: A :class:`~mixprop.data.scaler.StandardScaler` fitted on the data.
    :param features_scaler: A :class:`~mixprop.data.scaler.StandardScaler` fitted on the features.
    :param atom_descriptor_scaler: A :class:`~mixprop.data.scaler.StandardScaler` fitted on the atom descriptors.
    :param bond_feature_scaler: A :class:`~mixprop.data.scaler.StandardScaler` fitted on the bond_fetaures.
    :param args: The :class:`~mixprop.args.TrainArgs` object containing the arguments the model was trained with.
...
def load_checkpoint(path: str,
    ...
    :return: The loaded :class:`~mixprop.models.model.MoleculeModel`.
    """
    if logger is not None:
        debug, info = logger.debug, logger.info
    else:
        debug = info = null #= print # Edited to quiet the loading process
...
def load_scalers(path: str) -> Tuple[StandardScaler, StandardScaler, StandardScaler, StandardScaler]:
    ...
    :return: A tuple with the data :class:`~mixprop.data.scaler.StandardScaler`
             and features :class:`~mixprop.data.scaler.StandardScaler`.
    """
    state = torch.load(path, map_location=lambda storage, loc: storage)
...
def load_args(path: str) -> TrainArgs:
    ...
    :param path: Path where model checkpoint is saved.
    :return: The :class:`~mixprop.args.TrainArgs` object that the model was trained with.
    """
    args = TrainArgs()
...
def build_optimizer(model: nn.Module, args: TrainArgs) -> Optimizer:
    ...
    :param args: A :class:`~mixprop.args.TrainArgs` object containing optimizer arguments.
...
def build_lr_scheduler(optimizer: Optimizer, args: TrainArgs, total_epochs: List[int] = None) -> _LRScheduler:
    ...
    :param args: A :class:`~mixprop.args.TrainArgs` object containing learning rate arguments.
...
def save_smiles_splits(data_path: str,
    ...
    :param train_data: Train :class:`~mixprop.data.data.MoleculeDataset`.
    :param val_data: Validation :class:`~mixprop.data.data.MoleculeDataset`.
    :param test_data: Test :class:`~mixprop.data.data.MoleculeDataset`.
...
def update_prediction_args(predict_args: PredictArgs,
    ...
    :param predict_args: The :class:`~mixprop.args.PredictArgs` object containing the arguments to use for making predictions.
    :param train_args: The :class:`~mixprop.args.TrainArgs` object containing the arguments used to train the model previously.
    :param missing_to_defaults: Whether to replace missing training arguments with the current defaults for :class: `~mixprop.args.TrainArgs`.
